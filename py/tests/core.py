...
class TestLoop(unittest.TestCase):
	...
	def testSubstepping(self):
		...
		O.engines = [
		        PyRunner(
		                initRun=True,
		                iterPeriod=1,
		                command='if O.subStep!=0: raise RuntimeError("O.subStep!=0 inside the loop with O.subStepping==False!")'
		        )
		]
		O.step()
	...
	def testEnginesModificationInsideLoop(self):
		...
		O.engines = [
		        PyRunner(initRun=True, iterPeriod=1,
		                 command='from yade import *; O.engines=[ForceResetter(),GravityEngine(),NewtonIntegrator()]'),  # change engines here
...
class TestMaterials(unittest.TestCase):
	...
	def setUp(self):
		...
		O.reset()
		O.materials.append([FrictMat(young=1, label='materialZero'), ElastMat(young=100, label='materialOne')])
		O.bodies.append([utils.sphere([0, 0, 0], .5, material=0), utils.sphere([1, 1, 1], .5, material=0), utils.sphere([1, 1, 1], .5, material=1)])
...
"""
	...
	def testPythonToCppToPythonThrow(self):
		...
		O.engines += [
		        PyRunner(
		                command=
		                "from yade import pack;pred=pack.inHyperboloid(centerBottom=(0,0,-.1),centerTop=(0,0,.1),radius=.05,skirt=.03);pack.randomDensePack(pred,spheresInCell=100,radius=8e-2)",
		                iterPeriod=1
		        )
		]
		self.assertRaises(RuntimeError, lambda: O.run(5, True))
