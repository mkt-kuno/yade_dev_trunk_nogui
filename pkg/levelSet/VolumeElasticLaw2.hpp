...
public:
	...
	// clang-format off
		YADE_CLASS_BASE_DOC_ATTRS_CTOR_PY(Law2_VolumeGeom_FrictPhys_Elastic,LawFunctor,"Contact law for elasticity, scaling linearly with the overlap volume, and Mohr-Coulomb plastic failure without cohesion.\nThis law implements a volumetric variant of the classical linear elastic-plastic law from [CundallStrack1979]_ (see also [Pfc3dManual30]_). The normal force is (with the convention of positive tensile forces) $F_n=\\min(k_n V_{overlap}, 0)$. The shear force is $F_s=k_s u_s$, the plasticity condition defines the maximum value of the shear force : $F_s^{\\max}=F_n\\tan(\\phi)$, with $\\phi$ the friction angle.",
