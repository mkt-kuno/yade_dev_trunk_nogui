...
for i in range(Ngrid):
    ...
    grid[i, :] = diameterPartI/2 * np.array([np.cos(theta) * np.sin(phi), 
...
for i in range(nbIntruders):
    O.bodies.append(sphere( 
        center=((i+0.5)*length/nbIntruders, 
        		leftLimitI, 
        		bottomPos+depositionHeightBelowI+depositionHeightI/2), 
...
def saveData():
    ...
    if step == Nstep:
        ...
        for k, intr in enumerate(intruders):
            ax0.plot(time, intr.averagedPos[:,2]*1000, '-', color=cmap(k),
                        label="intruder "+str(k)+" (averaged)")
            ax0.plot(time, intr.instantPos[:,2]*1000,  '-', color=cmap(k), alpha=0.2,
                        label="intruder "+str(k)+" (intantaneous)")
        ...
        for k, intr in enumerate(intruders):
            ax1.plot(time, intr.averagedVel[:,0], '-', color=cmap(k),
                        label="intruder "+str(k)+" (averaged)")
            ax1.plot(time, intr.instantVel[:,0],  '-', color=cmap(k), alpha=0.2,
                        label="intruder "+str(k)+" (intantaneous)")
        ...
        for k, intr in enumerate(intruders):
            ax2.plot(time, intr.averagedForce[:,2], '-', color=cmap(k),
                        label="intruder "+str(k)+" (averaged)")
            ax2.plot(time, intr.instantForce[:,2],  '-', color=cmap(k), alpha=0.2,
                        label="intruder "+str(k)+" (intantaneous)")
        ...
        norm = mpl.colors.Normalize(vmin=min(FcnFieldMean), 
                                    vmax=max(FcnFieldMean))
        ...
        fig.colorbar(mpl.cm.ScalarMappable(norm=norm, cmap=cmap), 
                        ax=ax, aspect=60, pad=0.1, label="P / Pmax [-]")
