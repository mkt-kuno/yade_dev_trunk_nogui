...
X, Y, Z = numpy.meshgrid(numpy.linspace(-xMax, xMax, nGP[0]),
                         numpy.linspace(-yMax, yMax, nGP[1]),
                         numpy.linspace(-zMax, zMax, nGP[2])
                         ,indexing='ij')
distField = Y
O.bodies.append(levelSetBody(grid = grid, distField = distField.tolist(), dynamic = False)) # because we know here that surface nodes will not express on that LS floor (bigger than the sphere), we do not need to pay attention to those and n_neighborsNodes

# The LS sphere:
lsSph = O.bodies.append(levelSetBody('sphere',(-0.9*xMax,1.9,0),0.5,spacing = 0.05, nSurfNodes = 1602, n_neighborsNodes = 6)) # mind the n_neighborsNodes assignment
...
O.engines = [
    ...
    ,InteractionLoop(
        ...
        ,warnRoleIg2 = False) # mind the warnRoleIg2 = False
...
def saveData():
    if O.interactions.has(0, 1, True):
        ...
        nodeIdx = nan
        un = nan
    plot.addData(it = O.iter, pos = lsSph.state.pos,
                 velLin = lsSph.state.vel,
                 velAng = lsSph.state.angVel,
                 un = un, nodeIdx = nodeIdx)
