def phiFromStl(stlFile,nMin=80,extendCoeff=1):
    ...
    gpArrayReshaped = gpArray.reshape(-1,3) # converting in correct format (with as many lines as necessary and 3 columns for x,y,z) as expected by trimesh below
    # Running trimesh to conclude:
    phiValues = -trimesh.proximity.signed_distance(mesh,gpArrayReshaped) # "-" for being positive outside, negative inside; with proximity.closest_point as the under-the-hood-instrumental function
